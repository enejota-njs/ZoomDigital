module address_block_average (
    ...
    reg [2:0] state = IDLE;
    reg [8:0] row, col;
    reg [8:0] out_row, out_col;  // Linha e coluna na imagem reduzida
    reg [16:0] base_addr;
    reg [1:0] read_delay;
    ...
    always @(*) begin
        ...
        col = (in_address % 160) * 2;  // Coluna base na imagem original: 0, 2, 4, ..., 318
        row = (in_address / 160) * 2;  // Linha base na imagem original: 0, 2, 4, ..., 238
        base_addr = row * 320 + col;   // Endereço na memória 320x240
        
        // Calcula posição na imagem reduzida (para memória 320x240)
        out_row = in_address / 160;    // Linha na imagem reduzida: 0-119
        out_col = in_address % 160;    // Coluna na imagem reduzida: 0-159
    end
            ...
            case (state)
                ...
                READ_11: begin
                    if (read_delay > 0) begin
                        ...
                        out_address <= out_row * 320 + out_col;
                        state <= WRITE;
                    end
                end
